...
network_bootstrap = function(y, X, N, bootstrap_time, index, data, link = 'probit', beta_NULL = NULL, boot_repeat = 0){
  ...
  # eta_MLE = matrix_to_panel_df(DGP$eta)$X
...
#' @export
get_APE_analytical_ameri<- function(y, X, N, data, index, fit, L = 1, model = 'probit'){
  K = dim(X)[2]
  APE_MLE = rep(0, K)
  ...
  for (k in 1:K) {
    X = X_to[,k]
    cof_estimate = fit$est$coefficients[k]
    cof_estimate_MLE = fit$est_MLE$coefficients[k]
    eta = t(fit$est$eta)
    eta_MLE = t(fit$est_MLE$eta)
    if (is.numeric(X) && length(unique(X)) == 2) {
