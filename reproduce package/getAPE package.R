...
getAPEs <- function(
  ...
  binary <- apply(X, 2L, function(x) all(length(unique(x)) == 2))
  ...
  for (j in seq.int(p)) {
    if (binary[[j]]) {
      eta0 <- eta - X[, j] * beta[[j]] + beta[[j]] * min(X[, j])
      eta1 <- eta - X[, j] * beta[[j]] + beta[[j]] * max(X[, j])
      f1 <- family[["mu.eta"]](eta1)
      Delta[, j] <- (family[["linkinv"]](eta1) - family[["linkinv"]](eta0)) / (max(X[, j]) - min(X[, j]))
      Delta1[, j] <- (f1 - family[["mu.eta"]](eta0)) / (max(X[, j]) - min(X[, j]))
  ...
  Psi <- - Delta1 / w
  MPsi <- CenterVariables(Psi, w, k.list, control[["center.tol"]]) # residual
  PPsi <- Psi - MPsi # fitted
  rm(Delta1, Psi)
  ...
  if (biascorr) {
    ...
    for (j in seq.int(p)) {
      if (binary[[j]]) {
        eta0 <- eta - X[, j] * beta[[j]] + beta[[j]] * min(X[, j])
        etain = eta - X[, j] * beta[[j]] + beta[[j]] * max(X[, j])
        Delta2[, j] <- (partialMuEta( etain , family, 2L) - partialMuEta(eta0, family, 2L))/ (max(X[, j]) - min(X[, j]))
        rm(eta0)
      } else {
